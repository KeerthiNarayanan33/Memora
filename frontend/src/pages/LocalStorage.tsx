      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="badge bg-blue-50 text-blue-700 border border-blue-200 text-xs font-semibold">
              Local Storage Repository
            </span>
          </div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">On-Device Storage Confinement</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="card p-5 bg-white border-slate-200 shadow-xs">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-slate-500 uppercase font-semibold">Total Stored</span>
            <HardDrive className="w-4 h-4 text-brand-600" />
          </div>
          <div className="text-2xl font-extrabold text-slate-900 font-mono">
            {stats ? `${stats.total_size_mb} MB` : '...'}
          </div>
          <div className="text-2xs text-slate-500 mt-1">Across all local vaults</div>
        </div>

        <div className="card p-5 bg-white border-slate-200 shadow-xs">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-slate-500 uppercase font-semibold">Files On-Disk</span>
            <FileText className="w-4 h-4 text-purple-600" />
          </div>
          <div className="text-2xl font-extrabold text-slate-900 font-mono">
            {stats ? stats.file_count : '...'}
          </div>
          <div className="text-2xs text-slate-500 mt-1">Audio, transcripts & DB</div>
        </div>

        <div className="card p-5 bg-white border-slate-200 shadow-xs">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-slate-500 uppercase font-semibold">Confinement</span>
            <Lock className="w-4 h-4 text-emerald-600" />
          </div>
          <div className="text-2xl font-extrabold text-emerald-600">ENFORCED</div>
          <div className="text-2xs text-slate-500 mt-1">Zero cloud sync default</div>
        </div>

        <div className="card p-5 bg-white border-slate-200 shadow-xs">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-slate-500 uppercase font-semibold">Storage Engine</span>
            <Database className="w-4 h-4 text-blue-600" />
          </div>
          <div className="text-lg font-extrabold text-slate-900">SQLite + Local FS</div>
          <div className="text-2xs text-slate-500 mt-1">PostgreSQL ready schema</div>
        </div>
      </div>

      {/* Directory Hierarchy Tree */}
      <div className="card p-6 md:p-8 bg-white border-slate-200 shadow-xs space-y-6">
        <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
          ...
          <span className="font-mono text-xs text-slate-500 font-normal">
            ({stats?.root_path || './MemoraData'})
          </span>
        </h2>

        <div className="bg-slate-900 p-6 rounded-xl border border-slate-800 font-mono text-xs text-slate-200 space-y-2 overflow-x-auto shadow-inner">
